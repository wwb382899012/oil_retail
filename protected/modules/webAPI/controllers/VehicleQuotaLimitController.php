...
class VehicleQuotaLimitController extends WebAPIController
{
    public function pageInit()
    {
        parent::pageInit(); // TODO: Change the autogenerated stub
        $this->rightCode = 'vehicleDayQuota';
    }
    /**
     * @api {POST} /webAPI/VehicleQuotaLimit/list [list]车辆每日限额列表
     * @apiName list
     * @apiGroup WebAPI - VehicleQuotaLimit
     * @apiVersion 1.0.0
     ...
     * @apiParam (输出字段-data-data) {string} code 车辆限额编号
     * @apiParam (输出字段-data-data) {string} rate 当日油箱占比
    ...
    /**
     * @api {GET} /webAPI/VehicleQuotaLimit/detail [detail]获取每日限额详情
     * @apiName detail
     * @apiGroup WebAPI - VehicleQuotaLimit
     * @apiVersion 1.0.0
     * @apiExample {json} 输入示例:
     ...
     * @apiParam (输出字段-data) {string} code 车辆限额编号
     * @apiParam (输出字段-data) {string} rate 当日油箱占比
    ...
    public function actionDetail()
    {
       /* $id = Mod::app()->request->getParam('limit_id');
        ...
        //车辆每日限额
        try{
            $service = new \app\ddd\Quota\Application\VehicleQuotaLimit\VehicleQuotaLimitService();
            $vehicleQuotaLimit=$service->getActiveVehicleQuotaLimit();

            $VehicleQuotaLimitDTO = new \app\ddd\Quota\DTO\VehicleQuotaLimit\VehicleQuotaLimitDTO();
            $VehicleQuotaLimitDTO->fromEntity($vehicleQuotaLimit);

            $this->returnSuccess($VehicleQuotaLimitDTO->getAttributes());
        }catch(Exception $e){
    ...
    /**
     * @api {POST} /webAPI/VehicleQuotaLimit/add [add]车辆每日限额 新增
     * @apiName add
     * @apiGroup WebAPI - VehicleQuotaLimit
     * @apiVersion 1.0.0
     * @apiParam  (输入字段){string} rate 当日油箱占比 <font color=red>必填</font>
     *
     * @apiExample {json} 输入示例:
     * {
     * "rate"=>'0.6'
    ...
    public function actionAdd()
    {
        ...
        if(empty($rate)){
            $this->returnError('车辆当日可用额度不能为空');
        }
        if(!empty($rate)&&!is_numeric($rate)){
            $this->returnError('车辆当日可用额度不能为非数字');
        }


        try{

            $servive = new \app\ddd\Quota\Application\VehicleQuotaLimit\VehicleQuotaLimitService();
