 ...
 * Describe：
 */
class AttachmentController extends WebAPIController
{
    /**
     * 附件类型
     * @var string
    ...
    public function actionSaveFile()
    {
        ...
        $res=$obj->saveFile($id,$type,$_FILES["files"],$userId,$extras,$this->isWordToPdf);
        ...
        if($res==1){
            $this->returnSuccess([
                ...
                "type"=> $type,
                "name"=> $obj->file["name"],
                "status"=>$obj->file["status"],
                "url"=>$obj->file["fileUrl"]
            ]);
        }
        else
