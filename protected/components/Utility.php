...
class Utility
{
    ...
    {
        return DbUtility::beginTransaction($dbType);
    }
    ...
    {
        return DbUtility::query($sql, $dbType , $params);
    }
    ...
    {
        return DbUtility::queryOneNumber($sql, $fieldName);
    }
    ...
    {
        return DbUtility::queryIsEmpty($sql);
    }
    ...
    {
        return DbUtility::queryInMaster($sql,$dbType);
    }
    ...
    {
        return DbUtility::execute($sql,$dbType,$params);
    }
    ...
    {
        return DbUtility::executeWithNoTransaction($sql,$dbType);
    }
    ...
    {
        return DbUtility::executeSql($sql,$dbType,$params);
    }
    ...
    {
        return DbUtility::isInDbTrans($dbType);
    }
    ...
    {
        return DbUtility::filterInject($str);
    }
    ...
    {
        return DbUtility::getMainDbName();
    }
    ...
    {
        return CheckUtility::isEmpty($data);
    }
    ...
    {
        return CheckUtility::isNotEmpty($data);
    }
    ...
    {
        return UserUtility::getNowUserId();
    }
     ...
     * @return mixed
     */
    public static function getNowUserName()
    {
        return UserUtility::getNowUserName();
    }
    ...
    public static function getNowUserRoles() {
        return UserUtility::getNowUserRoleIds();
    }
    ...
    public static function checkNowUserIsAdmin() {
        return UserUtility::isAdminOfCurrentUser();
    }
     ...
     * @return null
     */
    public static function hValsCache($keyName)
    {
        if(empty($keyName))
            return null;
        $redis = Mod::app()->redis;
        $s = $redis->hvals($keyName);
        return $s;
    }
    ...
    public static function fen2yuan($fen) {
        ...
        /*if (is_int($fen) || ctype_digit($fen)) {
            ...
            BusinessException::throw_exception(RetailError::$FEN_TO_YUAN_ERR, array('fen' => $fen));
        }*/
    }
            ...
            if (preg_match('/^[0-9]+.[0-9]$/', $yuan)) {
                ...
                if (preg_match('/^[0-9]+.[0-9][0-9]$/', $yuan)) {
                    ...
                    if (!is_string($yuan)) {//非字符串
                        BusinessException::throw_exception(RetailError::$NOT_STANDARD_YUAN, array('yuan' => $yuan));
                    } else {
                        BusinessException::throw_exception(RetailError::$NOT_STANDARD_YUAN, array('yuan' => $yuan));
                    }
                }
        ...
        if (PHP_INT_SIZE === 4) {//32位机器
            if ($y < 21474836) {
                ...
                if ($y == 21474836 && $f <= 47) {
                    ...
                    BusinessException::throw_exception(RetailError::$YUAN_TO_FEN_ERR, array('yuan' => $yuan, 'num' => '2147483647L'));
                }
            }
        } else {
            if (PHP_INT_SIZE === 8) {//64位机器
                if ($y < 92233720368547758) {
                    ...
                    if ($y == 92233720368547758 && $f <= 07) {
                        ...
                        BusinessException::throw_exception(RetailError::$YUAN_TO_FEN_ERR, array('yuan' => $yuan, 'num' => '9223372036854775807'));
                    }
                }
            } else {
                BusinessException::throw_exception(RetailError::$UNSUPPORT_COMPUTER);
            }
        }
    }
     ...
     * @return array|mixed
     */
    public static function postCMD($params,$url)
    {
        $curl = Mod::app()->curl;
        try{
            $res = $curl->post($url, $params);
            Mod::log("PostCMD Log, params is ".json_encode($params).", and result is ".$res);
            if(empty($res))
        ...
        {
            Mod::log("PostCMD Log, params is ".json_encode($params).", and error message is ".$e->getMessage(),"error");
            return array("code"=>-1,"data"=>$e->getMessage());
        }
    ...
    {
        $fullName = is_object($class) ? get_class($class) : $class;
        return basename(str_replace('\\', '/', $fullName));
    }
}