...
class BaseActiveRecord extends CActiveRecord
{
    ...
    /**
     * 保存，如果保存失败时不需要抛出异常，则传第三个参数为false
    ...
    public function save($runValidation=true,$attributes=null)
    {
        ...
        try {
            ...
            if(!$res) {
                ...
                {
                    Mod::log(__CLASS__ . '->' . __FUNCTION__ . ' in line ' . __LINE__ . '，' . $this->tableName() . '保存失败: ' . $error[0]  . '，' . $field . ':' . $this->getAttribute($field), "error");
                }
    ...
    /**
     * 分页
     * @param array $params CDbCriteria属性数组
     * @param int $page
     * @param int $pagesize
     * @return array
     */
    public function findAllByPageToArray($params = [], $page = 0,$pagesize = 10)
    {
        ...
        $rows = array_map(function($record) { return $record->attributes;}, $data);
