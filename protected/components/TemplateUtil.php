    ...
    public static function parseTemplateRecursion($template, $template_len, $values, &$result, &$start_pos) {
        while (true) {
            ...
            if ($pos_key_end === false) {
                BusinessException::throw_exception(RetailError::$NOT_FIND, array('key' => '}', 'msg' => ''));
            }
            $key = substr($template, $pos + 2, $pos_key_end - ($pos + 2));
            if (!array_key_exists($key, $values)) {
                BusinessException::throw_exception(RetailError::$NOT_IN_VALUE, array('key' => $key));
            }
            $result .= $values[$key];
        ...
        if (Utility::isNotEmpty($matches[1])) {
            foreach ($matches[1] as $key => $row) {
                ...
                if (array_key_exists($row, $values)) {
                    ...
                    BusinessException::throw_exception(RetailError::$NOT_IN_VALUE, array('key' => $row));
                }
            }
        }
    }
